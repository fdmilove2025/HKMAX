...
def get_stock_info(ticker):
    ...
    try:
        ...
        if metrics is None or "error" in metrics:
            error_msg = (
                metrics.get("error", f"Could not retrieve information for {ticker}")
                if metrics
                else f"Could not retrieve information for {ticker}"
            )
        ...
        return (
            jsonify({"error": f"An error occurred while retrieving information for {ticker}. Please try again later."}),
            500,
        )
