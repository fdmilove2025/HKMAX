...
"""
    ...
    try:
        ...
        if asset_classes and tickers and names and descs and len(asset_classes) == len(tickers) == len(names) == len(descs):
...
def generate_dynamic_securities_with_llm(user_data, portfolio_allocation):
    ...
    try:
        ...
        elif user_data.get('riskReaction') == 'Panic and Sell' or user_data.get('returnPreference') == 'Stable Predictable Returns':
    ...
    for asset_class in asset_classes:
        ...
        if class_match:
            ...
            for match in re.finditer(name_ticker_pattern, class_text):
                ...
                description = desc_match.group(1).strip() if desc_match else f"A {asset_class.lower()} investment option."
