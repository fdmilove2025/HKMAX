...
def filter_stocks(query=None, exchange=None, limit=100):
    ...
    if query:
        ...
        filtered_stocks = [
            ...
            if (stock.get('symbol') is not None and query in stock.get('symbol', '').lower()) or 
               (stock.get('name') is not None and query in stock.get('name', '').lower())
        ]
    ...
    if exchange:
        ...
        filtered_stocks = [
            stock for stock in filtered_stocks
            if (stock.get('exchange') is not None and exchange in stock.get('exchange', '').lower()) or 
               (stock.get('exchangeShortName') is not None and exchange in stock.get('exchangeShortName', '').lower())
        ]
    ...
    try:
        ...
        try:
            ...
            if not portfolio:
                ...
                return {
                    "error": "No portfolio found for your account. Please complete the questionnaire first."
                }
        except Exception as e:
            print(f"Database error retrieving portfolio for user_id {user_id}: {e}")
            return {
                "error": "Failed to retrieve your portfolio data. Please try again later."
            }
        ...
        try:
            ...
            if not llm_response or len(llm_response) < 10:  # Verify we got a meaningful response
                ...
                return {
                    "error": "Analysis service returned an empty response. Please try again later."
                }
        except Exception as e:
        ...
        try:
            ...
            if json_match:
                ...
                if missing_fields:
                    ...
                    return {
                        "error": f"Analysis is incomplete. Please try again later."
                    }
                ...
                return {
                    "error": "Could not extract valid response from analysis service. Please try again later."
                }
        except json.JSONDecodeError as e:
            print(f"JSON parse error for {ticker}: {e}")
            print(f"Raw LLM response: {llm_response[:300]}...")
            return {
                "error": "Failed to parse analysis results. Please try again later."
            }
    except Exception as e:
        print(f"Unexpected error in stock fit analysis for {ticker}: {e}")
        return {
            "error": f"An unexpected error occurred while analyzing {ticker}. Please try again later."
        }
...
def get_stock_metrics(ticker):
    ...
    try:
        ...
        is_etf = (profile.get('industry') == 'ETF' or 
                 'ETF' in profile.get('companyName', '') or 
                 'ETF' in quote.get('name', '') or
                 quote.get('name', '').endswith('ETF'))
