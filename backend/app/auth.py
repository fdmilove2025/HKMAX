...
def register():
    ...
    try:
        ...
        # Create new user
        new_user = User(email=data["email"], username=data["username"], age=age, has_faceid=data.get("faceid", False))
        ...
        return jsonify(
            {
                ...
                "user": {"id": user.id, "username": user.username, "email": user.email, "has_faceid": user.has_faceid},
                "access_token": access_token,
            }
        )
...
def login():
    ...
    try:
        ...
        if user.is_two_factor_enabled:
            ...
            return (
                jsonify({"message": "2FA required", "2fa_required": True, "temp_access_token": temp_access_token}),
                200,
            )
