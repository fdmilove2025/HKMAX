...
const Register = () => {
  ...
  const [showFacialSetupModal, setShowFacialSetupModal] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Show facial setup modal instead of immediately navigating
        setShowFacialSetupModal(true);
      } else {
  ...
  const handleFacialSetup = async (enableFacial) => {
    if (enableFacial) {
      // Navigate to facial registration page
      navigate("/facial-registration", { replace: true });
    } else {
      // Navigate to login page after user makes their choice
      navigate("/login", {
        replace: true,
        state: {
          message:
  ...
  return (
    <div className="min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8 pt-20">
      {/* Facial Setup Modal */}
      {showFacialSetupModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-dark-100 rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
            <div className="text-center">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">
                Enable Facial Login?
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                Would you like to set up facial recognition for faster and more
                secure login? You can always enable this later in your account
                settings.
              </p>
              <div className="flex flex-col space-y-3">
                <button
                  onClick={() => handleFacialSetup(true)}
                ...
                >
                  Yes, enable facial login
                </button>
                <button
                  onClick={() => handleFacialSetup(false)}
